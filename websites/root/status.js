...
const updateMargin = () => {
	if (ready && statusElement != null) {
		const style = getComputedStyle(statusElement);
		document.body.style.marginTop = (Number(style.height.replace("px", "")) + Number(style.paddingTop.replace("px", "")) + Number(style.paddingBottom.replace("px", "")) + 6) + "px";
	}
};
...
const updateStatus = () => {
	if (ready && status != null) {
		...
		else if (status.startDate) statusElement.innerHTML += `<div class="date">From ${formatDate(status.startDate)}</div>`;
