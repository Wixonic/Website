...
const RichLink = (url, el) => {
	...
	const hide = () => hoverEl.style.opacity = "";
	...
	engine.push(async () => {
		const result = await request("GET", new URL(`/rich/link?url=${encodeURIComponent(url)}`, localEnvironment ? path.local.functions : path.functions), "json", "application/json", null, -1);
