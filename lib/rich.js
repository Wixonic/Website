 ...
 * @param {boolean} richpopup
 * @param {boolean} forcerich
 * @returns {HTMLAnchorElement}
 */
const RichLink = (url, el, popup = true, force = false) => {
	if (!el) el = document.createElement("a");

	el.classList.add("richlink");
	el.href = url.toString();

	if (force || ((localEnvironment ? (!url.hostname.endsWith("localhost") || url.host.endsWith("localhost:2014")) : (!url.hostname.endsWith("wixonic.fr") || url.hostname.endsWith("go.wixonic.fr"))) && !el.classList.contains("norichpopup") && popup)) {
		const data = {
			title: null,
			description: null,
			thumbnail: null,
		...
		engine.push(async () => {
			const result = await request("GET", new URL(`${localEnvironment ? "/wixonic-website-2/europe-west1/httpServer" : ""}/rich/link?url=${encodeURIComponent(url)}`, localEnvironment ? path.local.functions : path.functions), "json", "application/json", null, 24 * 60 * 60);
