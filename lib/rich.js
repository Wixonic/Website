 ...
 */
const RichLink = (url, el, richpopup = true) => {
	if (!el) el = document.createElement("a");

	el.classList.add("richlink");
	el.href = url.toString();

	if ((localEnvironment ? (!url.hostname.endsWith("localhost") || url.host.endsWith("localhost:2014")) : (!url.hostname.endsWith("wixonic.fr") || url.hostname.endsWith("go.wixonic.fr"))) && !el.classList.contains("norichpopup") && richpopup) {
		const data = {
			title: null,
			description: null,
			thumbnail: null,
