...
const init = async () => {
	...
	const termsOfUse = await RichLink(new URL("/terms", localEnvironment ? path.local.root : path.root));
	termsOfUse.classList.add("fade");
	termsOfUse.innerHTML = "Terms of Use";
	legalLinks.append(termsOfUse);

	legalLinks.innerHTML += " | ";

	const privacyPolicy = await RichLink(new URL("/privacy", localEnvironment ? path.local.root : path.root));
