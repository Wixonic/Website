...
firebase.getUser = async (force) => {
	...
	try {
		const req = await request("GET", new URL("/auth/token/", localEnvironment ? path.local.functions : path.functions), "json", "application/json", null, -1, true);
...
firebase.signInWithEmail = async (email, password) => {
	try {
		...
		const response = await request("POST", new URL("/auth/token/", localEnvironment ? path.local.functions : path.functions), "json", "application/json", JSON.stringify({
			idToken
		}), -1, true);
...
firebase.signOut = async (reload = true) => {
	try {
		...
		await request("DELETE", new URL("/auth/token/", localEnvironment ? path.local.functions : path.functions), "text", null, null, -1, true);
