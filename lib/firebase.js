...
const functions = getFunctions(app, window.localEnvironment ? "http://localhost:2013" : "https://functions.wixonic.fr");
...
const authEngine = {
	...
	changeEmail: (email) => {
		...
		return new Promise(async (resolve, reject) => {
			const data = await request("POST", new URL("/email/change", functions.customDomain), "text", null, body, null, true);
	...
	signInWithEmail: (email, password) => {
		...
		return new Promise(async (resolve, reject) => {
			const data = await request("POST", new URL("/email", functions.customDomain), "text", null, body, null, true);
	...
	signInWithSession: async () => {
		return new Promise(async (resolve, reject) => {
			const data = await request("GET", new URL("/session", functions.customDomain), "text", null, null, null, true);
			if (data.status == 200) signInWithCustomToken(auth, data.response)
...
const init = async () => {
	try {
		...
		console.warn(`Failed to login using cookies: ${e}`);
	}
};
